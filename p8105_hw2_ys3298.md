...
trash_wheel = read_excel("../data/HealthyHarborWaterWheelTotals2018-7-28.xlsx", sheet = 1, range = "A2:N338") %>%
  janitor::clean_names() %>%
  filter(dumpster != "NA") %>%
...
year by year. For trash\_wheel dataset, there are 285 observations.
Variables from this dataset include “year”, “month”, “date”, numbers of
“plastic bottles”, “glass bottles” etc. For precipitation dataset,
there are 19 observations(excluding the total number for whole year in
2017 and 2018.). Variables include year, month and total number of
precipitation in each month in 2017 and 2018.

The median numbers of sports balls in a dumpster in 2014-2018 are 6, 16,
26, 8, 3. The mean numbers of plastic bottle in a dumpster in 2014-2018
are 1540, 1960, 2980, 2260, 995. The mean numbers of grocery bags in
2014-2018 are 880, 1392, 2292, 1939, 676. From these data, we can easily
noticed that the number of sports balls, plastic bottle and grocery bags
firstly increased and then decrease from 2014 to 2018.
...
pols = read_csv("../data/pols-month.csv") %>%
  ...
  mutate(month = month.abb[as.factor(month)],
         president = factor(prez_gop, labels = c("dem", "gop", "others(shown as 2 in prez_gop)"))) %>%
...
```
    ...
    ## # … with 1,381 more rows, and 3 more variables: day <int>, close <dbl>,
    ## #   unemployment <dbl>
